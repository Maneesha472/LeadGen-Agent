...
BLOCKED_DOMAINS = {
    ...
    "sulekha.com", "glassdoor.co.in", "ambitionbox.com", "goodfirms.co", "clutch.co", "justdial.com"
}

def _discover_websites_google(query: str, max_results: int) -> list:
    websites = []
    seen_domains = set()
    try:
        print(f"Querying Google for: {query}")
        results = search(query, num_results=max_results * 2, lang="en")
        for actual_url in results:
...
if __name__ == "__main__":
    results = _discover_websites_google("Software Development companies in Hyderabad", 10)
    print("\nRESULTS:")
    for r in results:
        print(r)
