export class ProgressEvent {
    static itemCompleted(input) {
        ...
        const item = { item_id: input.itemId, state: 'completed' };
        if (input.thumbnailUrl)
            item.thumbnail_url = input.thumbnailUrl;
        if (input.displayName)
            item.display_name = input.displayName;
        const payload = {
        ...
        if (input.focusItemId)
            payload.focus_item_id = input.focusItemId;
        if (input.detail)
            payload.detail = input.detail;
        return payload;
    }
    static milestone(input) {
        const milestone = { id: input.id, state: input.state };
        if (input.label)
            milestone.label = input.label;
        const payload = { event_type: 'milestone', milestone };
        if (input.detail)
            payload.detail = input.detail;
        return payload;
    }
    static unitCharge(input) {
        ...
        const unitChargeTelemetry = {};
        if (input.amountInrPaise !== undefined)
            unitChargeTelemetry.reported_cost_inr_paise = input.amountInrPaise;
        else if (currency.toUpperCase() === 'INR' && input.amount !== undefined)
            unitChargeTelemetry.reported_cost_inr_paise = Math.trunc(input.amount);
        else {
        ...
        if (input.unitCount !== undefined)
            unitChargeTelemetry.unit_count = input.unitCount;
        if (input.unit !== undefined)
            unitChargeTelemetry.unit = input.unit;
        if (input.metadata !== undefined)
            unitChargeTelemetry.metadata = input.metadata;
