...
async function runContractTest(endpoint, args) {
...
async function signedDiscoverCheck(endpoint, options) {
    const response = await postContract(endpoint, '/discover', {}, options, 'contract-test-discover');
    if (response.status >= 500)
        return { name: 'signed_discover', passed: false, detail: `HTTP ${response.status}`, remediation: 'Keep /discover lightweight and non-blocking.' };
    if (response.status >= 400)
        return { name: 'signed_discover', passed: false, detail: `HTTP ${response.status}`, remediation: 'Verify signing credentials and endpoint URL.' };
...
async function idempotencyCheck(endpoint, options) {
...
async function staleSignatureCheck(endpoint, options) {
...
async function malformedPrepareCheck(endpoint, options) {
...
async function postContract(endpoint, path, payload, options, idempotencyKey, timestamp) {
...
function contractUrl(endpoint, path) {
