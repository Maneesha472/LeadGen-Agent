...
export function validateContractMessage(kind, payload) {
    const schema = CONTRACT_SCHEMAS[kind];
    if (!schema)
        return { valid: false, errors: [{ field_path: '', message: 'unknown contract message kind', received: kind }] };
    const errors = [];
...
function validate(schema, value, path, errors) {
    ...
    if (schema.type === 'object') {
        ...
        for (const field of schema.required ?? []) {
            if (!(field in source))
                errors.push({ field_path: joinPath(path, field), message: 'required field missing', expected: schema.properties?.[field]?.type, received: 'missing' });
        }
        for (const [field, child] of Object.entries(schema.properties ?? {})) {
            if (field in source)
                validate(child, source[field], joinPath(path, field), errors);
        }
    }
    if (schema.type === 'array') {
        ...
        if (schema.items)
            source.forEach((item, index) => validate(schema.items, item, joinPath(path, String(index)), errors));
    }
}
