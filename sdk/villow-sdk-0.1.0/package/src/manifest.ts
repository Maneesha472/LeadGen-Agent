...
const templateSchema = z
  ...
  .refine(
    (template) =>
      template.unit_ceiling === undefined ||
      template.typical_charge === undefined ||
      template.typical_charge <= template.unit_ceiling,
    { message: 'typical_charge must not exceed unit_ceiling', path: ['typical_charge'] },
  );
...
export function validateManifest(path: string): ManifestValidationResult {
  try {
...
function manifestWarnings(manifest: z.infer<typeof manifestSchema>): string[] {
  ...
  manifest.task_templates.forEach((template, index) => {
    ...
    if (template.artifact_review_primitive && deferredReviewPrimitives.has(template.artifact_review_primitive)) {
      warnings.push(
        `task_templates[${index}].artifact_review_primitive ${template.artifact_review_primitive} is deferred from V1 launch and will render with the simple typed fallback`,
      );
    }
  });
