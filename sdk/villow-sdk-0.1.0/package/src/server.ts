...
export function createApp(agent: Agent): FastifyInstance {
  ...
  app.post('/discover', (request, reply) =>
    signedEndpoint(agent, request, reply, async () => ({
  ...
  app.post('/prepare', (request, reply) =>
    signedEndpoint(agent, request, reply, async (payload) => {
  ...
  app.post('/preview', (request, reply) =>
    signedEndpoint(agent, request, reply, async (payload) => {
      const taskTemplate = requiredString(payload, 'task_template');
      const taskId = requiredString(payload, 'task_id');
  ...
  app.post('/execute', (request, reply) =>
    signedEndpoint(agent, request, reply, async (payload) => {
      const taskTemplate = requiredString(payload, 'task_template');
      const taskId = requiredString(payload, 'task_id');
      ...
      if (ctx.nextAgentState !== null) response.agent_state = ctx.nextAgentState;
      if (response.agent_state && typeof response.agent_state === 'object' && !Array.isArray(response.agent_state)) {
        agent.taskAgentState.set(taskId, response.agent_state as Record<string, unknown>);
      }
      if (!agent.taskStates.has(taskId)) agent.taskStates.set(taskId, 'executing');
      return response;
    }),
  );

  app.post('/clarification_response', (request, reply) =>
    signedEndpoint(agent, request, reply, async (payload) => {
      const taskId = requiredString(payload, 'task_id');
  ...
  app.post('/result', (request, reply) =>
    signedEndpoint(agent, request, reply, async (payload) => {
      ...
      if (agentState !== undefined) response.agent_state = agentState;
      return response;
    }),
  );
...
async function signedEndpoint(agent: Agent, request: FastifyRequest, reply: FastifyReply, handler: Handler): Promise<void> {
...
function contextFor(agent: Agent, payload: Record<string, unknown>, taskId: string, taskTemplate: string, inputs: Record<string, unknown>): Context {
  return new Context({
    agent,
    taskId,
    taskTemplate,
    ...
    toolAccessGrants: Array.isArray(payload.tool_access_grants) ? payload.tool_access_grants as Array<Record<string, unknown> | string> : [],
...
function recordContextEvents(agent: Agent, taskId: string, ctx: Context): void {
...
function requiredHandler(handlers: Map<string, (...args: unknown[]) => unknown>, taskTemplate: string): (...args: unknown[]) => Promise<Record<string, unknown>> | Record<string, unknown> {
  const handler = handlers.get(taskTemplate);
  if (!handler) throw new Error(`unknown_task_template:${taskTemplate}`);
  return handler as (...args: unknown[]) => Promise<Record<string, unknown>> | Record<string, unknown>;
}
...
function rawRequestBody(body: unknown): Buffer {
...
function parseBody(body: Buffer | unknown): Record<string, unknown> {
